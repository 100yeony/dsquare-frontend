  ...
  <v-card class="mx-auto justify-center" variant="outlined" @click="handleCardClicked">
    <v-card-item>
      <div>
        <v-row>
          <v-col>
            <div class="text-caption font-0000008F">{{ props?.data?.question?.name }}({{
              !props?.data?.question?.teamHierarchy ? [] :
        ...
        <div class="mt-2 mb-3 text-caption font-0000008F">내 답변: {{ props?.data?.content.replace(/<\/?[^>]+(>|$)/g, " ") }}</div>
      </div>
      <v-row>
