              ...
              <v-container class="text-center font_white_gray font-xs">
                ...
                <div class="mt-5">
                  ...
                  <v-text-field v-model="qData.commentText" :ref="qData.commentInputRef" type="input" variant="outlined"
                    ...
                    @click:append-inner="writeComment(qData, 'talk', qnaId)"></v-text-field>
                </div>
              </v-container>
  ...
  mounted() {
    ...
    questionData.then(
      async (response) => {
        ...
        let res = await this.callComments('talk', this.qnaId)
        Object.assign(this.qData, {
          commentInputRef: 'talk' + this.qnaId,
          mentionName: '',
          mentionWriterId: 0,
          mentionId: 0,
          commentMode: false,
    ...
    async toggleLike(board, id) {
      ...
      if ([200, 201].includes(res.status)) {  // 성공
        if (this.qData.likeYn) {
          this.qData.likeCnt--;
          if (store.getters["info/infoPageState"]?.talkCardData) {
            var storedItem = store.getters["info/infoPageState"]?.talkCardData?.find(post => post.talkId == id);
            storedItem.likeCnt--;
