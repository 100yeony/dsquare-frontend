...
var methods = {
    ...
    phoneRegex(val, model, size = phoneMaxLength) {
      if (phoneExe.exec(val) !== null || model.length > size) {
        return model = model.slice(0, -1);
      } else {
        return model;
      }
    ...
    phoneRegexCheck(val){
        if(val.length >= phoneMinLength && val.length <= phoneMaxLength) {
